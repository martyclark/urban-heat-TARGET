...
STATIC_IP_NAME="hit-static-ip"     # reserved IP name (permanent, survives VM deletion)
# ─────────────────────────────────────────────────────────────────────────────

log() { echo "▶ $*"; }

REGION="${ZONE%-*}"   # europe-west2-b → europe-west2
...
# ── Reserve static external IP (survives VM stop/delete) ─────────────────────
...
log "  http://$STATIC_IP:8501"
log ""
log "Startup takes ~3 min on first boot (Docker install + image pull)."
log "To watch:  gcloud compute ssh $VM_NAME --zone=$ZONE -- tail -f /var/log/hit-startup.log"
log ""
log "── On-demand use (stop when not needed) ────────────────────────────────"
log "  Stop:   gcloud compute instances stop  $VM_NAME --zone=$ZONE"
log "  Start:  gcloud compute instances start $VM_NAME --zone=$ZONE"
log "  URL stays the same: http://$STATIC_IP:8501"
log ""
log "── After testing: archive the VM as a machine image ────────────────────"
log "  1. gcloud compute machine-images create hit-app-image \\"
log "         --source-instance=$VM_NAME --source-instance-zone=$ZONE"
log "  2. gcloud compute instances delete $VM_NAME --zone=$ZONE"
log "  Monthly cost then: ~\$5 (image storage) + ~\$0 (static IP while in-use)"
log ""
log "── Recreate VM from image when needed ───────────────────────────────────"
log "  gcloud compute instances create $VM_NAME \\"
log "      --zone=$ZONE --machine-type=$MACHINE \\"
log "      --source-machine-image=hit-app-image \\"
log "      --address=$STATIC_IP_NAME"
