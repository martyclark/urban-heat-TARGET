...
class Target:
    ...
    def run_simulation(self, save_csv=False, paras=None):
        ...
        self.met_data.index = pd.to_datetime(self.met_data.index, format=self.cfM['date_fmt'])
        ...
        met_data_all = met_data_all.reset_index(drop=True)  # pandas 3.0: integer positional index for met[col][i] lookups
        ...
        ##########################################################################################
        self.mod_rslts = mod_rslts[1:len(met_data_all)]  ### THIS IS THE FINAL DATA ARRAY WITH MODEL OUTPUTS  ######
        ##########################################################################################
    ...
    def __init_validation(self):
        ...
        if not self.__validated:
            ...
            self.obs_data.index = pd.to_datetime(self.obs_data.index, format=self.cfM['date_fmt'])
